...
def test_register_cc_skill_idempotent(tmp_path):
    ...
    register_cc_skill("plugin-creator", tmp_path)
    files = list((tmp_path / ".claude" / "skills" / "plugin-creator").iterdir())
    assert len(files) == 1
...
def test_register_codex_skill_idempotent(tmp_path):
    ...
    register_codex_skill("plugin-creator", tmp_path)
    files = list((tmp_path / ".codex" / "skills" / "plugin-creator").iterdir())
    assert len(files) == 1
...
def test_register_cc_skill_does_not_break_0mvp_register_cc(tmp_path):
    ...
    register_cc_skill("plugin-creator", tmp_path)
    # Both files exist (original plugin-harness + new plugin-creator)
