...
AC4: python -c "from src.assembler.plan import assemble; from src.schema.state import InterviewState; s=InterviewState(); s.set_answer('what-who-where','# heading\\n[link](http://x)\\n![img](http://x)\\n`code`\\n*em*\\n_str_\\n> quote\\n<script>alert(1)</script>'); [s.set_answer(q,'x'*30) for q in ['why-this-problem','how-it-works','ai-usage','how-verified']]; out=assemble(s); for tok in ['# heading\\n','[link](http://x)','![img]','*em*','_str_','> quote','<script>']: assert tok not in out or tok.replace('<','&lt;').replace('>','&gt;').replace('#','&#35;').replace('*','\\*').replace('_','\\_').replace('`','\\`').replace('[','\\[').replace(']','\\]') in out" → exit 0 (all 8 escape vectors neutralized: heading, link, image, code-span, emphasis, underscore, blockquote, HTML)
```
